...
jQuery(document).ready(function($){
	...
	YRC.auth = {
		...
		'url': function uuu(type, page, res, search, limit, vst){
			...
			switch(type){
				...
				break;
				case 'Uploads':
					url = this.baseUrl(['se', 'ar', 'c', 'h'].join('')+'?order='+ ((search || ['vi', 'ew', 'C', 'ou', 'nt'].join(''))) +'&q='+(vst.t)+'&part=snippet'+( vst.own ? ('&channelId='+ res) : '') +'&type=video&pageToken='+page+'&maxResults='+limit);
				break;
				case 'channel':
	...
	YRC.Setup.prototype = {
		...
		'events': function(){
			...
			$('body').on('click', '.yrc-player-bar .yrc-close span', function(e){ yc.closePlayer(e); });
	...
	YRC.template.video = function( vid, res, style ){
		...
		return '<li class="yrc-video yrc-item-'+ cl +' yrc-item pb-inline yrc-just-listed" data-video="'+ vid_id +'">\
			<a href="'+ watch_video + vid_id +'" class="yrc-video-link pb-block">\
	...
	YRC.run = function(shell){
		if(!shell.attr('data-yrc-setup') && shell.length){
			...
			new YRC.Setup(YRC.counter++, shell.data('yrc-channel'), shell);return;
		}
	};
